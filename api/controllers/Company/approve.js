...
const sendEmailToCompany = (company) => {
  ...
  EmailService.sendToUser({ email: _.get(company, 'email') }, "company-is-approved-email", contentData);
};
...
module.exports = async function (req, res) {
  ...
  if (company.status !== PENDING && company.status !== REJECTED) {
    return res.badRequest({
      message: 'Cannot execute this action. Company is not pending and not rejected.',
      devMessage: 'Company status is not pending and not rejected.',
